    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            function drawRow(rowData) {
                ...
                var deletes
                if (rowData['has_requests'] == true) {
                    var deletes = '<button id="btnDeletes" type="button" class="btn btn-outline-secondary" deleterow"><i class="icofont-ui-delete text-danger"></i></button>'
                }
                else {
                    var deletes = '<button id="btnDelete" type="button" class="btn btn-outline-secondary" value=' + rowData["id"] + ' deleterow"><i class="icofont-ui-delete text-danger"></i></button>'
                }
...
$("#branchForm").validate({
    ...
    submitHandler: function (e) {
        ...
        if (editId != 0) {
            $.ajax({
                url: "/api/official/branch/" + editId + "/",
                type: "PUT",
                data: data,
                beforeSend: function (xhr) {
                    xhr.setRequestHeader(
...
$(document).on('click', '#btnDeletes', function () {
    swal("Oops! The account cannot be deleted,Your service request is still pending.", {
        icon: "error",
    });
});
